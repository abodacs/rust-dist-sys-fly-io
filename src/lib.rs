...
pub fn main_loop<S, N, P, IP>(init_stat: S) -> anyhow::Result<()>
where
    ...
    IP: Send + 'static,
{
    ...
    let mut node: N =
        Node::from_init(init_stat, init, tx.clone()).context("Node intilized fialed")?;
    ...
    jh.join().expect("Thread panaic").context("")?;
